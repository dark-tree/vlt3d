...
class ShaderModule {
	...
	public:
		...
		ShaderModule(Device& device, const uint32_t* data, uint32_t size, Kind kind, const char* name = nullptr)
		: vk_stage((VkShaderStageFlagBits) kind.vulkan) {
			...
			VulkanDebug::name(device.vk_device, VK_OBJECT_TYPE_SHADER_MODULE, vk_module, name);
